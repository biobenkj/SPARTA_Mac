...
class CheckDependencies(object):
    ...
    def getNumPy(self):
        ...
        subprocess.call(["mv", "download\?source\=files", "numpy-latest.tar.gz"])
        subprocess.call(["tar", "-zxf", "numpy-latest.tar.gz"])
        subprocess.call(["rm", "numpy-latest.tar.gz"])
        return

    def installNumPy(self):
        """Install latest NumPy from source"""

        current_numpy = glob.glob("numpy-*")
    ...
    def getSPARTAdir(self):
        ...
        sparta_dir = os.path.join(desk_path, "SPARTA")
        if not os.path.lexists(desk_path):
            print "Unable to find the Desktop."
        while not os.path.lexists(sparta_dir):
            sparta_dir = str(raw_input("SPARTA folder is not on the Desktop. Please place the folder on the Desktop or enter the file path for the folder location or enter quit to exit the program:"))
            if sparta_dir.upper() == "Q" or sparta_dir.upper() == "QUIT":
                quit()
            if not os.path.lexists(sparta_dir):
                print("Invalid file path. The path you have selected does not exist or was not written correctly. \nAn example of path on Mac OS X: /Users/yourusername/Desktop/SPARTA")
        return sparta_dir